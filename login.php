...
<form action="<?php echo htmlentities($_SERVER["PHP_SELF"]); // Redirect to the page itself ?>" method="post">
    <table class="table table-sm table-borderless">
        <tr>
            <td><label for="user">Username:</label></td>
            <td><input type="text" name="user" class="form-control" id="user" value="<?php if ($_SERVER["REQUEST_METHOD"] == "POST") echo $_POST["user"]; // If the server sent a POST request when accessing the page, then show the previously inputted username ?>"></td>
        </tr>
        <tr>
